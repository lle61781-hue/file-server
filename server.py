...
# ============================================================
# ROUTES BỔ SUNG CHO CHỨC NĂNG FILE
# ============================================================

@app.route('/download', methods=['POST'])
@login_required
def download_file_route():
    """Tạo URL tải xuống có thể dùng được từ Cloudinary."""
    try:
        ...
        # Sử dụng Cloudinary utility để tạo URL. 
        # attachment=True: buộc trình duyệt/client tải xuống file
        ...
        create_activity_log('DOWNLOAD_FILE', f'Tải file: {file_record.filename}', target_user_id=file_record.user_id)
...
def log_file_opened(public_id):
    """
    FIX LỖI HTTP 400: Ghi log hành động mở file và cập nhật last_opened_at/by.
    Client gửi public_id (đã được Flask tự động giải mã).
    """
    try:
        data = request.get_json()
...
def update_file_sync():
    """Nhận file đã thay đổi từ client và cập nhật Cloudinary."""
    ...
    if not file_record:
        return jsonify({'message': 'File không tồn tại trên server.'}), 404
        
    # File được gửi đến để thay thế file cũ, giữ nguyên tên và ID
    uploaded_file = request.files['file']
    
    try:
        # Sử dụng API Cloudinary để upload lại file, giữ nguyên public_id
        ...
        # Trả về version (mặc dù version không được theo dõi trong DB của bạn, Cloudinary cung cấp nó)
        return jsonify({'message': 'File đã được đồng bộ thành công!', 'version': upload_result.get('version', 1)}), 200
        
    except Exception as e:
        logger.error(f"Error processing file update sync for {public_id}: {e}")
